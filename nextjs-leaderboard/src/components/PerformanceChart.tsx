...
import { ScatterChart, Scatter, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, Radar } from 'recharts';
...
export default function PerformanceChart({ data }: PerformanceChartProps) {
  ...
  const scatterData: ScatterDataPoint[] = validData.map(entry => ({
    x: entry.malidup_f1!,
    y: entry.malisam_f1!,
    model: entry.model,
    type: entry.type,
  ...
  const availableModels = data.filter(entry => 
    entry.malidup_f1 !== null || entry.malisam_f1 !== null
  );
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white dark:bg-gray-800 p-3 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg">
          <p className="font-semibold text-gray-900 dark:text-white">{data.model}</p>
          <p className="text-sm text-gray-600 dark:text-gray-400">{data.organization}</p>
          <p className="text-sm text-gray-600 dark:text-gray-400">Type: {data.type}</p>
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Compare MALIDUP F1 (true homologs) vs MALISAM F1 (non-homologs). Higher is better for both metrics.
        </p>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 20, right: 20, bottom: 80, left: 60 }}>
                ...
                <XAxis 
                  ...
                  label={{ value: 'MALIDUP F1 (True Homologs) ↑', position: 'insideBottom', offset: -15 }}
                />
                <YAxis 
                  type="number" 
                  dataKey="y" 
                  ...
                  domain={['dataMin - 0.01', 'dataMax + 0.01']}
                  tickFormatter={(value) => value.toFixed(2)}
                  label={{ value: 'MALISAM F1 (Non-homologs) ↑', angle: -90, position: 'insideLeft' }}
                />
                <Tooltip content={<CustomTooltip />} />
                <Legend />
                {Array.from(new Set(scatterData.map(d => d.type))).map(type => (
                  <Scatter
                    key={type}
                    name={type}
