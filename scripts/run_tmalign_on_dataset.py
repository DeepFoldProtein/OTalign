...
def load_alignments_from_jsonl(path: str | Path) -> List[Dict[str, Any]]:
    """Loads alignment results from a JSONL file."""
...
def _worker(task):
    ...
    try:
        # Define paths
        fasta_path = Path(args["fasta_dir"]) / f"{pair_id}.fasta"
        if not fasta_path.exists():
            return {"pair_id": pair_id, "error": "missing_fasta"}
        ...
        # Run TMalign with the given alignment
        cmd = [
            args["tmalign_bin"],
            str(pdb1_path),
            str(pdb2_path),
        ...
        if "metrics" not in rec:
            rec["metrics"] = {}
        rec["metrics"].update(tm_scores)
        rec["meta"] = rec.get("meta", {})
        rec["meta"]["tmalign_mode"] = "guided"
...
def main():
    ap = argparse.ArgumentParser(description="Run TMalign on a dataset using pre-computed alignments from FASTA files.")
    ap.add_argument("--alignments_jsonl", type=str, required=True, help="Path to the JSONL file with alignment predictions.")
    ap.add_argument("--fasta_dir", type=str, required=True, help="Directory containing *.fasta alignment files.")
    ...
    ap.add_argument("--extra_args", nargs="*", default=[], help="Additional flags for TMalign (e.g., -a T).")
    ...
    alignments = load_alignments_from_jsonl(args.alignments_jsonl)
    ...
    tasks = ((ex, args_dict) for ex in alignments)
    ...
    print(f"[ok] wrote TMalign scores -> {out_path}")
