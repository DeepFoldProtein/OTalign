import argparse
import glob
import json
import os
from pathlib import Path

from tqdm.auto import tqdm


def iter_jsonl(path: Path):
    """Yields JSON objects from a JSONL file."""
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            yield json.loads(line)


def main():
    ap = argparse.ArgumentParser(description="Prepare MALISAM PDBs by creating a flat directory of symlinks named by seq_id.")
    ap.add_argument(
        "--malisam_jsonl",
        type=str,
        required=True,
        help="Path to the MALISAM JSONL file (e.g., from convert_malisam.py).",
    )
    ap.add_argument(
        "--pdb_root",
        type=str,
        required=True,
        help="Root directory of the original MALISAM PDB files (e.g., data/MALISAM).",
    )
    ap.add_argument(
        "--output_dir",
        type=str,
        required=True,
        help="Directory to save the flattened symlinks (e.g., data/MALISAM_flat).",
    )
    args = ap.parse_args()

    malisam_jsonl = Path(args.malisam_jsonl)
    pdb_root = Path(args.pdb_root)
    output_dir = Path(args.output_dir)

    output_dir.mkdir(parents=True, exist_ok=True)

    print(f"Creating symlinks in {output_dir}...")

    created_count = 0
    skipped_count = 0

    for ex in tqdm(iter_jsonl(malisam_jsonl), desc="Processing pairs"):
        group_id = ex.get("group_id")
        pair_id = ex.get("pair_id")
        if not group_id or not pair_id:
            continue
        try:
            original_ali_path = Path(glob.glob(str(pdb_root / group_id / "*.manual.ali"))[0])
            aln1, aln2 = original_ali_path.read_text().upper().strip().splitlines()

            output_path = output_dir / f"{pair_id}.fasta"
            output_path.write_text(f">seq1\n{aln1}\n>seq2\n{aln2}")
        except Exception as e:
            print(f"An error occurred for seq_id '{group_id}': {e}")
            skipped_count += 1

        for seq_key in ["seq1_id", "seq2_id"]:
            seq_id = ex.get(seq_key)
            if not seq_id:
                continue

            try:
                # Expected seq_id format: 'dir_name:scop_id'
                # e.g., 'd1a05a_d1dgsa3:d1a05a_'
                dir_name, scop_id = seq_id.split(":")

                # Extract PDB ID and chain, e.g., d1a05a_ -> "1a05"
                if len(scop_id) < 6:
                    print(f"Warning: Invalid scop_id format, skipping: {scop_id}")
                    skipped_count += 1
                    continue
                pdb_id_part = scop_id[1:5]

                # Search for a file like '1a05_*.pdb'
                source_dir = pdb_root / dir_name
                glob_pattern = f"{pdb_id_part}_*.pdb"
                found_files = list(source_dir.glob(glob_pattern))

                if len(found_files) == 1:
                    original_pdb_path = found_files[0]
                else:
                    print(f"Warning: Found {len(found_files)} files for pattern '{glob_pattern}' in {source_dir}. Skipping.")
                    skipped_count += 1
                    continue

                # Symlink path: {output_dir}/{seq_id}.pdb
                symlink_path = output_dir / f"{seq_id}.pdb"

                # Create relative symlink for portability
                relative_original_path = os.path.relpath(original_pdb_path.resolve(), symlink_path.parent.resolve())

                if not symlink_path.exists():
                    os.symlink(relative_original_path, symlink_path)
                    created_count += 1
                else:
                    # If it exists but points to the wrong place, update it
                    if not symlink_path.is_symlink() or os.readlink(symlink_path) != str(relative_original_path):
                        os.remove(symlink_path)
                        os.symlink(relative_original_path, symlink_path)
                        created_count += 1

            except ValueError:
                print(f"Warning: Could not parse seq_id '{seq_id}', skipping.")
                skipped_count += 1
            except Exception as e:
                print(f"An error occurred for seq_id '{seq_id}': {e}")
                skipped_count += 1

    print(f"\n[ok] Done. Created {created_count} new symlinks. Skipped {skipped_count} entries.")
    print(f"Flat PDB directory ready at: {output_dir}")


if __name__ == "__main__":
    main()
